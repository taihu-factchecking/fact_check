...
【回應格式】：回應必須是字典列表形式，每個字典包含兩個鍵：「claim」：提取的單位事實；「clause」：claim對應到文本的子句（可包含一或多句），代表claim是從文本的那些子句中提取出事實的，一定要與文本的某個子字串相符、一模一樣，不要把不相關的子句包含進來，且clause之間不要有重疊的部分；此外，如果子句的前面有連接詞，且包含進來不影響文義，則一併包含進來。
