    ...
    async def _call_gpt(text: str) -> str:
        ...
        responses = await client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""
給定一段文本，你的任務是準確識別並依序提取文本中每一個聲明（claim），並標註聲明對應到文本的原始句子（可為一或多句）。聲明是一段事實的敘述，並可由人類驗證為真或假。為確保每個聲明清晰無歧義，應避免使用代詞或其他指代表達，且每個聲明應簡潔並獨立存在。
【聲明提取規則】：
1. 每個聲明必須是清晰、可獨立驗證的事實，不可包含模糊的推論。
2. 聲明須為完整的事實，沒有失去其在文本中的上下文背景。
